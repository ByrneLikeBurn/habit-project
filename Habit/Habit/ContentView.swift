...
struct ContentView: View {
    ...
    /// next render.
    ///
    /// Restored after briefly being removed on a suspicion, during the
    /// check-off bug investigation, that this API was somehow involved —
    /// it wasn't; the actual cause was a missing `.contentShape` on
    /// `HabitRow`'s controls (see below). Noting that here so the same
    /// suspicion doesn't resurface.
    ...
    var body: some View {
        HStack(spacing: 15) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                // checking the persisted store directly. A flat,
                // non-repeated group of buttons (the sort-mode chips) never
                // had this problem; only rows with two-or-more sibling
                // controls did.
